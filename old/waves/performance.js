...
function calculateGPUScore(gl) {
    ...
    score +=
        (extensions.filter((ext) => gl.getExtension(ext)).length /
            extensions.length) *
        0.5;

    const maxTexSize = gl.getParameter(gl.MAX_TEXTURE_SIZE);
    score +=
        Math.min(Math.max((maxTexSize - 2048) / (8192 - 2048), 0), 1) * 0.3;
...
export function createPerformanceTracker(gl) {
    ...
    function updateResolution(frameDuration) {
        ...
        const viewportScore = Math.min(
            (gl.canvas.width * gl.canvas.height) / (1920 * 1080),
            1,
        );
        const performanceScore = Math.min(16.67 / avgFrameTime, 1);

        const baseResolution =
            RESOLUTION_LIMITS.min +
            (RESOLUTION_LIMITS.max - RESOLUTION_LIMITS.min) *
                Math.min(gpuScore * 0.6 + viewportScore * 0.4, 1) *
                performanceScore;

        const newResolution = Math.round(
            currentResolution * 0.8 + baseResolution * 0.2,
        );
