...
export function createAnimationSystem(
    ...
    function animate(context) {
        ...
        drawParams.backgroundColor.forEach(
            (channel, i) =>
                (drawParams.color[i] = (channel * drawParams.brightness) / 255),
        );

        drawParams.ratio =
            Math.max(
                1.0,
                Math.min(context.viewportWidth / context.viewportHeight, 2.0),
            ) * 0.375;
        drawParams.time =
            drawParams.time + (context.time - lastTime) * drawParams.flowSpeed;
